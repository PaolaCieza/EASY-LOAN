<?php
    ...
    switch ($opcion){
        case 0:
            ...
            cliente c on r.idcliente=c.idcliente where s.idcliente=$cliente";
            break;
        case 1:
            $sql="SELECT c.nombre ||' '||c.apellido as prestamista, c.fotousuario, p.fecharegistro, p.monto,p.numerocuotas, p.tasainteres*100 as interes,
            (case when p.estado=true then 'Pagado' else 'Pendiente' end) estado from prestamo p inner join respuesta r 
            on r.idrespuesta=p.idrespuesta inner join solicitud s on s.idsolicitud=r.idsolicitud inner join 
            cliente c on r.idcliente=c.idcliente where s.idcliente=$cliente order by p.monto asc";
            break;
        case 2:
            $sql="SELECT c.nombre ||' '||c.apellido as prestamista, c.fotousuario, p.fecharegistro, p.monto,p.numerocuotas, p.tasainteres*100 as interes,
            (case when p.estado=true then 'Pagado' else 'Pendiente' end) estado from prestamo p inner join respuesta r 
            on r.idrespuesta=p.idrespuesta inner join solicitud s on s.idsolicitud=r.idsolicitud inner join 
            cliente c on r.idcliente=c.idcliente where s.idcliente=$cliente order by p.monto desc";
            break;
        case 3:
            $sql="SELECT c.nombre ||' '||c.apellido as prestamista, c.fotousuario, p.fecharegistro, p.monto,p.numerocuotas, p.tasainteres*100 as interes,
            (case when p.estado=true then 'Pagado' else 'Pendiente' end) estado from prestamo p inner join respuesta r 
            on r.idrespuesta=p.idrespuesta inner join solicitud s on s.idsolicitud=r.idsolicitud inner join 
            cliente c on r.idcliente=c.idcliente where s.idcliente=$cliente and p.estado=false";
            break;
        case 4:
            $sql="SELECT c.nombre ||' '||c.apellido as prestamista, c.fotousuario, p.fecharegistro, p.monto,p.numerocuotas, p.tasainteres*100 as interes,
            (case when p.estado=true then 'Pagado' else 'Pendiente' end) estado from prestamo p inner join respuesta r 
            on r.idrespuesta=p.idrespuesta inner join solicitud s on s.idsolicitud=r.idsolicitud inner join 
            cliente c on r.idcliente=c.idcliente where s.idcliente=$cliente and p.estado=true";
            break;
    };
